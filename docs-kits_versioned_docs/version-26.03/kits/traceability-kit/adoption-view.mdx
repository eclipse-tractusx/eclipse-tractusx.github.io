...
Quality issues not only appear while vehicles are on the road. During the assembling phase at the OEM components can show unexpected performances or abnormalities which needs to be analysed. Until the root cause isn't clear and the problem is not solved, these components must be blocked.
...
This DT can be used as well as a fleet DT by an OEM. All vehicles in field or in an earlier stage that show the same abnormality or want to be observed can be linked to it (like a BoM), marked by that as affected and analysed deeper.
