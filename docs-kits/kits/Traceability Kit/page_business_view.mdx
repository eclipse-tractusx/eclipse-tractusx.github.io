...
As the quality notifications, block notifications are a form of data exchange to transfer information about faulty parts in a standardized way. In contrast to quality notifications in general and specially quality alerts, which are currently used in particular for the (first) contact between business partners in order to initiate a self disclosure, block notifications are a form of immediate measure to report critical faults in parts directly to business partners in real time and to force the affected parts to be blocked in the next step. 

In this way, the customer can react quickly and precisely locate the parts based on the block information and sort them out at an early stage in order to prevent subsequent damage or major recalls and thus save costs. Block notifications should therefore significantly simplify the data exchange of block information in a standardized way and improve speed and quality.
