...
| customerPartId | Optional | The ID of the type/catalog part from the _customer_.  <br />The main reason why this propertiy is optional is that it cannot be guaranteed that every manufacturer knows the customerPartId for their parts. In case the manufacturer knows the customer and the corresponding CustomerPartID of its part though, it is _required_ to add this information for easier lookup and to enable further processes. | String |
...
The Submodel Descriptor for Parquet file information can be defined like:
      ...
      "endpoints": [
        {
          "interface": "SUBMODEL-3.0",
          "protocolInformation": {
             "href": "https://connector.data.plane/api/public/parquet/urn%3Auuid%3A7effd7f4-6353-4401-9547-c54b420a22a0",
            "endpointProtocol": "HTTP",
            "endpointProtocolVersion": [
              "1.1"
            ],
...
| semanticId.keys[].value | urn:samm:io.catenax.binary_exchange:1.0.0#BinaryExchange                                                | Aspect model which provides information (dspAssetId, dataplane URl and controlplane URL) for AmazonS3 EDC asset   |
...
**Aspect model BinaryExchange can be found in Semantic Modeling repository [Github](https://github.com/eclipse-tractusx/sldt-semantic-models/)**
  ...
  ```json
{
  ...
  "dspInformation": {
    "controlPlaneUrl": "dspEndpoint=https://semantics-edc-provider-controlplane.catenax.net/api/v1/dsp",
