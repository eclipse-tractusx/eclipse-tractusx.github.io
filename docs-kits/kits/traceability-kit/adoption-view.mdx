...
Best practise of getting these data as a supplier is a data pull via digital twin (DT) qualityTask which is created by the OEM. The DT qualityTask (please see next section) reflects the quality issue itself and provides the digital mirror of the affected fleet at the same time. Alternative the information ca be provided via DT of the affected car. 
