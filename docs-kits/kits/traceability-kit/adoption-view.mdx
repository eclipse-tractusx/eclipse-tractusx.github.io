...
In this case it is crucial that the supplier is informed immediately. The information which is provided with the aspect model "zeroKmFailure" supports a fast start of processing, anaysing and identification of all affected components as well as solving the problem. 
