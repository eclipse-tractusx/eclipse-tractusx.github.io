...
### Aspect Model ZeroKmFailure

Quality issues not only appears while vehicles are on the road. During the assembling phase at the OEM components can show unexpected performances or abnormalities which needs to be analysed. Until the root cause isn't clear and the problem is not solved, these components must be blocked.
In this case it is crucial that the supplier is informed immediately. The information which is provided with the aspect model "zeroKmFailure" supports a fast start of processing, analysing and identification of all affected components as well as solving the problem. 
Information like the data matrix code (DMC) of the component, assembly line and date in combination of the amount of defect parts per shift and a description of apprearing error, and some more data, is transmitted by this aspect model. This bundle of data enables the Tier 1 to narrow down affected production batches much faster.

#### Processual Approach

Best practise of getting these data as a supplier is a data pull via digital twin (DT) qualityTask which is created by the OEM (the supplier is informed about an existing DT qualityTask due to a "unique ID Push" - see CX-0151 Industry Core Basics) by the OEM). The DT qualityTask reflects the quality issue itself and provides the digital mirror of the affected fleet at the same time. For more information about the DT qualityTask please see next section. 
Alternatively, the information can be provided via DT of the affected car. 
...
Furthermore the DT qualityTask provides the bridge to the data driven quality use case. Aspect models can be provided via DT from the OEM and pulled by the Tier1. To inform about a DT qualityTask, the unique ID push (see CX-0151 Industry Core Basics) mechanism can be used. 
...
This DT can be used as well as a fleet DT by an OEM. All vehicles in field or in an earlier stage which are showing the same abnormality or wants to be observed, can be linked to it (like a BoM), marked by that as affected and analysed deeper.
