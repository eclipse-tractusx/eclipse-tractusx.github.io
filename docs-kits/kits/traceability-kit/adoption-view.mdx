...
Information like the data matrix code (DMC) of the component, line and date in combination of defect parts per shift and a description of apprearing error and some more data is transmitted by this aspect model. These bundle of data enables the Tier 1 to narrowing down affected production batches much faster.

#### Processual Approach

Best practise of getting these data as a supplier is a data pull via digital twin (DT) qualityTask which is created by the OEM (the supplier is informed about an existing DT qualityTask due to a unique ID push by the OEM). The DT qualityTask (please see next section) reflects the quality issue itself and provides the digital mirror of the affected fleet at the same time. Alternative the information ca be provided via DT of the affected car. 
