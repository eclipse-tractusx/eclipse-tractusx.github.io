...
Quality issues not only appears while vehicles are on the road. During the assembling phase at the OEM components can show unexpected performances or abnormalities which needs to be analysed. Until the root cause isn't clear, and the problem is not solved, these components need to be blocked.
