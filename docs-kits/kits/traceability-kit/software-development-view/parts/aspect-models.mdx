...
The following section shows an overview of all aspect models of the Traceability Use Case that can be attached to part instance twins / qualityTask twins.
...
The ZeroKmFailure aspect model provides the essential information from the OEM to the supplier in case of anomalies or defects of delivered components during the assembly phase and enables the supplier to start analysing the root cause and identify all affected goods very fast.

Aspect model in GitHub:
- Version 1.0.0: https://github.com/eclipse-tractusx/sldt-semantic-models/tree/main/io.catenax.zero_km_failure/1.0.0

##### Example: Submodel `ZeroKmFailure`
