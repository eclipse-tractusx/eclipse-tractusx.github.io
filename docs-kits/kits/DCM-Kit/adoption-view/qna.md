...
|6|How often are Catena-X standards and in particular the DCM standard updated?|Catena-X standards can be updated on a quarterly basis, with breaking changes being allowed once a year. The DCM expert group is committed to introducing new features to the DCM standard without incurring breaking changes.|
...
|3|Can I directly send my data from Excel into the Catena-X dataspace?|No, you either require a business application that supports Excel upload or use the open source [Simple Data Exchanger](https://github.com/catenax-ng/tx-managed-simple-data-exchanger-frontend/tree/main/docs/user-guide), as soon as it supports DCM.|
