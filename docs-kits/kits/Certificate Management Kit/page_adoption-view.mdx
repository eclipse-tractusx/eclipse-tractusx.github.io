...
![Certificate Management kit banner](@site/static/img/BPKitIcon.png)

# Certificate Management KIT

** Unique Certificate Management data sets for the whole data space. **
## Vision & Mission
** Basis for the integration of value-adding services in the area of Certificate Management data management. **

The BusinessPartner Kit provides high-quality data records of Certificate Managements called golden record including a unique identifier and the Certificate Management number (BPN). The BPN provides a standardized and unique identifier on legal entity, site and address level. The connected backend service removes duplicate data, adds missing information, and improves data quality from different data sources so-called sharing member.
...
**The Kit provides a homogeneous semantic with an open data model, enables the access to high quality data and provides one access point to the Certificate Management data sets.**
<br/>
...
BPN is the unique number to identify and find partners in the Catena-X network. It will also provide high quality data sets from Certificate Managements, their legal unit, sites and addresses. 
...
For this, cutting individual company costs by offering the golden master data record as a shared service is the mission. And proactively reduce Certificate Management risk via collaborative value-add services.
More insights to the Certificate Management data management use case here: [Catena-X | Certificate Management Use Case](https://catena-x.net/fileadmin/user_upload/Standard-Bibliothek/5_PC_BPDM_v.2.1/BPM_-_001_BUSINESS_PARTNER_NUMBER_PlatformCapabilityBPDM_v2.1.pdf)
...
- Certificate Management Number (Version 2.0.0)
 
- Certificate Management Pool API (Version 2.0.0)
 
- Certificate Management Gate API (Version 2.0.0)
