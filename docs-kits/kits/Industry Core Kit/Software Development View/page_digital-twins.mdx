...
| :------------- | :----------- | :------------------------------------------------------------------------------------------- | :----- |
| partInstanceId | Mandatory    | The serial number of a part as specified by the manufacturer of this part. This must be the serial number that is applied to the part, e.g., as barcode or DMC etc., as the recipient, e.g., customer, of the part will use this information to lookup a digital twin of this part in the Digital Twin Registry of the manufacturer.<br/>A manufacturer might create more than one serial number for a part. For example, if there are customers that require the serial number to have a customer-specific format, the manufacturer might create two serial numbers for every part: an internal one that has a standard format for ease of use in the internal systems of a manufacturer, and a customer-specific one that is actually applied to the part. In this case, the customer-specific serial number is to be used as partInstanceId. | String |
