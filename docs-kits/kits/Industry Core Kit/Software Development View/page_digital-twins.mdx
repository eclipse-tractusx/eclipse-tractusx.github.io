...
To enforce a strict need-to-know (and prevent data from being exposed to non-authorized parties), the visibility of digital twins and entries in the attribute `specificAssetIds` of digital twins must be restricted to authorized partners  only. The actual implemenetation done depends on the DTR product used by the Catena-X partner. More details can be found in the [Digital Twin KIT](https://eclipse-tractusx.github.io/docs-kits/category/digital-twin-kit#Regulatory-Compliance-and-Security).
