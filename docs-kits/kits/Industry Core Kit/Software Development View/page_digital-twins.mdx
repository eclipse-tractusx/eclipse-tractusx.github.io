...
| :------------- | :----------- | :------------------------------------------------------------------------------------------- | :----- |
| partInstanceId | Mandatory    | The serial number of the part from the part's manufacturer. This must be the serial number that is applied to the part (as barcode or DMC) as the recipient (customer) of the part will use this information to lookup the part's digital twin in the manufacturer's DTR.<br/> A manufacturer might create more than one serial number for a part. For example, if there are customers that require the serial number to have a customer-specific format, the manufacturer might create two serial numbers for every part: an internal one that has a standard format for ease of use in the manufacturer's internal systems and a customer-specific one that is actually applied to the part. Only the customer-specific serial number is used as partInstanceId in this case. | String |
