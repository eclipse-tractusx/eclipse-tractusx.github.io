...
In general, three main building blocks for data exchange contracts are specified:
- FrameworkAgreement
- ContractReference
- UsagePurpose
...
```
The `rightOperand` here is a reference to an existing contract. Both party need to be able to identify a unique contract with the given reference.
...
```
The according `rightOperand`s for `UsagePurpose` depend highly on the use case and will be further documented in their individual KITs.

Versions for `UsagePurpose` are typically 1-digit.
