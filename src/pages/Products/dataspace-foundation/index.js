...
export default function DataspaceFoundationProductsPage() {
  const { siteConfig } = useDocusaurusContext();
  const category = productCategories.find(c => c.id === 'dataspace-foundation');
  const products = getProductsByCategory('dataspace-foundation');

  return (
    <Layout
      title={`Dataspace Foundation Products | ${siteConfig.title}`}
