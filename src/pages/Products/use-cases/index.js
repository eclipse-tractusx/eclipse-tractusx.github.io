...
export default function UseCasesProductsPage() {
  const { siteConfig } = useDocusaurusContext();
  const category = productCategories.find(c => c.id === 'use-cases');
  const products = getProductsByCategory('use-cases');

  return (
    <Layout
      title={`Use Cases Products | ${siteConfig.title}`}
