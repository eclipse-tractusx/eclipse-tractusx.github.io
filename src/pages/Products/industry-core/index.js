...
export default function IndustryCoreProductsPage() {
  const { siteConfig } = useDocusaurusContext();
  const category = productCategories.find(c => c.id === 'industry-core-foundation');
  const products = getProductsByCategory('industry-core-foundation');

  return (
    <Layout
      title={`Industry Core Foundation Products | ${siteConfig.title}`}
