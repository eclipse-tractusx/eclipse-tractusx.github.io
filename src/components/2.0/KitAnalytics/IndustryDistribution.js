...
export const IndustryDistribution = () => {
  ...
  // Extended color palette to support many industrys
  const colors = [
  ...
  const chartData = Object.entries(industryStats).map(([name, count], index) => ({
    label: name,
    value: count,
  ...
  return (
    <div className={styles.pieChartContainer} style={{
      ...
      <PieChart data={chartData} title="KITs by Industry Distribution" />
    </div>
  );
};
